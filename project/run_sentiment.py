...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        x_max = minitorch.max(x1, 2) + minitorch.max(x2, 2) + minitorch.max(x3, 2)
        x = self.linear.forward(x_max.view(x_max.shape[0], self.feature_map_size))
        if self.training:
            x = minitorch.dropout(x, self.dropout, False)
